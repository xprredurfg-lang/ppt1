...
SOURCE_CANDIDATES = [
    ROOT / 'source' / 'V4.docx',
    ROOT / '部门内训PPT文字稿（V4重构版）_重新发送_2026-02-26.docx',
]
TXT_CANDIDATES = [ROOT / 'source' / 'V4.txt']
...
def split_sections(text: str):
    if re.search(r'Slide\s*\d+\s*[｜|]', text, re.IGNORECASE):
        chunks = re.split(r'(?=Slide\s*\d+\s*[｜|])', text, flags=re.IGNORECASE)
        return [c.strip() for c in chunks if c.strip()]
    lines = [l.strip() for l in text.splitlines() if l.strip()]
    sections, cur = [], []
    for l in lines:
        if re.match(r'^(#|第[一二三四五六七八九十]+|\d+[\.、]).{2,}$', l) and cur:
            sections.append('\n'.join(cur))
            cur = [l]
        else:
            cur.append(l)
...
def main():
    ...
    out = build_slides(sections)
    (ROOT / 'slides.md').write_text(out, encoding='utf-8')
    print(f'Generated slides.md with {out.count("---") - 1} slides. Parsed sections: {len(sections)}')
